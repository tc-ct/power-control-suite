...
extern DAC_HandleTypeDef hdac_dac1; // SS1 (PA3)
...
/**
 * @brief  禁用内部参考电压，使 DAC 使用外部参考电压
 * @param  hdac DAC设备句柄
 * @retval HAL_OK  成功
 * @retval HAL_ERROR 参数错误或发送失败
 */
HAL_StatusTypeDef DAC_DisableInternalRef(DAC_HandleTypeDef* hdac)
{
	...
	if (hdac->type == DAC_TYPE_7568) {
		/* DAC7568 禁用内部参考命令: 32位 0x08000000
		 * 数据格式: 前缀[31:28]=0, 控制[27:24]=1001(参考命令),
		 * 地址[23:20]=0, 数据[19:0]=0, DB0=0
		 */
		uint32_t cmd_val = 0x08000000;
		buf[0] = (cmd_val >> 24) & 0xFF;
		buf[1] = (cmd_val >> 16) & 0xFF;
		buf[2] = (cmd_val >> 8) & 0xFF;
		buf[3] = cmd_val & 0xFF;
		cmd_len = 4;
	} else if (hdac->type == DAC_TYPE_7563) {
		/* DAC7563 禁用内部参考命令: 24位，命令=111 (DB21-19)，地址=000 (DB18-16)，数据=0，DB0=0
		 * 24位数值: (0x7 << 19) | (0 << 16) = 0x380000
		 */
		uint32_t cmd_val = (0x7 << 19) | (0 << 16);   // 24位值
		buf[0] = (cmd_val >> 16) & 0xFF;
		buf[1] = (cmd_val >> 8) & 0xFF;
		buf[2] = cmd_val & 0xFF;
		cmd_len = 3;
	} else
		return HAL_ERROR;
	...
	HAL_StatusTypeDef ret = HAL_SPI_Transmit(hdac->hspi, buf, cmd_len, SPI_TIMEOUT);
	HAL_GPIO_WritePin(hdac->cs_port, hdac->cs_pin, GPIO_PIN_SET);
...
HAL_StatusTypeDef DAC_SetVoltage(DAC_HandleTypeDef* hdac, uint8_t channel, uint16_t value)
{
	...
	hdac->tx_complete = 0;
	HAL_GPIO_WritePin(hdac->cs_port, hdac->cs_pin, GPIO_PIN_RESET);
	//HAL_SPI_TransmitReceive_DMA(hdac->hspi, hdac->tx_buf, hdac->rx_buf, data_size);
	HAL_StatusTypeDef status = HAL_SPI_Transmit(hdac->hspi, hdac->tx_buf, data_size, 100);
